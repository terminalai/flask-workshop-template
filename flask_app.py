...
def get():
    ...
    return {"user": f"<div class='container darker'><p class='user-msg'>{userText}</p><span class='time-right'>{time()}</span></div>", "bot": f"<div class='container'><p>{botText}</p><span class='time-left'>{time()}</span></div>"}
